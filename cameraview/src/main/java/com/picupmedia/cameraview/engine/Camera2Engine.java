...
import com.picupmedia.cameraview.CameraException;
import com.picupmedia.cameraview.CameraOptions;
import com.picupmedia.cameraview.PictureResult;
import com.picupmedia.cameraview.VideoResult;
...
import com.picupmedia.cameraview.engine.action.Action;
import com.picupmedia.cameraview.engine.action.ActionHolder;
import com.picupmedia.cameraview.engine.action.Actions;
import com.picupmedia.cameraview.engine.action.BaseAction;
import com.picupmedia.cameraview.engine.action.CompletionCallback;
import com.picupmedia.cameraview.engine.action.LogAction;
import com.picupmedia.cameraview.engine.mappers.Camera2Mapper;
import com.picupmedia.cameraview.engine.meter.MeterAction;
import com.picupmedia.cameraview.engine.meter.MeterResetAction;
import com.picupmedia.cameraview.engine.offset.Axis;
import com.picupmedia.cameraview.engine.offset.Reference;
import com.picupmedia.cameraview.engine.options.Camera2Options;
import com.picupmedia.cameraview.engine.orchestrator.CameraState;
import com.picupmedia.cameraview.frame.Frame;
import com.picupmedia.cameraview.frame.FrameManager;
import com.picupmedia.cameraview.frame.ImageFrameManager;
import com.picupmedia.cameraview.gesture.Gesture;
import com.picupmedia.cameraview.internal.CropHelper;
import com.picupmedia.cameraview.internal.FpsRangeValidator;
import com.picupmedia.cameraview.metering.MeteringRegions;
import com.picupmedia.cameraview.picture.Full2PictureRecorder;
import com.picupmedia.cameraview.picture.Snapshot2PictureRecorder;
import com.picupmedia.cameraview.preview.RendererCameraPreview;
...
import com.picupmedia.cameraview.video.Full2VideoRecorder;
import com.picupmedia.cameraview.video.SnapshotVideoRecorder;
...
public class Camera2Engine extends CameraBaseEngine implements
    ...
    private final CameraCaptureSession.CaptureCallback mWhiteBalanceRepeatingRequestCallback = new CameraCaptureSession.CaptureCallback() {
        @Override
        public void onCaptureStarted(@NonNull CameraCaptureSession session,
                                     @NonNull CaptureRequest request,
                                     long timestamp,
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session,
            ...
            if (awbMode != null && awbMode == CaptureResult.CONTROL_AWB_MODE_AUTO) {
                ...
                if (rggbChannelVector != null) {
                    getCallback().dispatchWhiteBalance(rggbChannelVector);
                }
            }
        }
    };
    ...
    public void toggleWhiteBalance(Boolean toggle) {
        XLogger.d("toggleWhiteBalance:" + toggle);
        try {
            if (toggle) {
                mRepeatingRequestBuilder.set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO);
                mSession.setRepeatingRequest(mRepeatingRequestBuilder.build(), mWhiteBalanceRepeatingRequestCallback, null);
            } else {
                mSession.setRepeatingRequest(mRepeatingRequestBuilder.build(), mRepeatingRequestCallback, null);
            }
        } catch (CameraAccessException e) {
            XLogger.d("open or close white balance error:" + e.getMessage());
        }
    }
    ...
    public void setTemperature(RggbChannelVector rggbChannelVector) {
        try {
            mRepeatingRequestBuilder.set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_OFF);
            // adjust color correction using seekbar's params
            mRepeatingRequestBuilder.set(CaptureRequest.COLOR_CORRECTION_MODE, CaptureRequest.COLOR_CORRECTION_MODE_TRANSFORM_MATRIX);
            mRepeatingRequestBuilder.set(CaptureRequest.COLOR_CORRECTION_GAINS, rggbChannelVector);
            mSession.setRepeatingRequest(mRepeatingRequestBuilder.build(), mRepeatingRequestCallback, null);
        } catch (CameraAccessException e) {
            XLogger.d("set temperature error:" + e.getMessage());
        }
    }
