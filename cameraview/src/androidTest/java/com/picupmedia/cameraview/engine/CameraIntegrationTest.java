...
import com.picupmedia.cameraview.CameraUtils;
import com.picupmedia.cameraview.CameraView;
import com.picupmedia.cameraview.PictureResult;
import com.picupmedia.cameraview.TestActivity;
...
import com.picupmedia.cameraview.tools.Emulator;
...
import com.picupmedia.cameraview.tools.Retry;
import com.picupmedia.cameraview.tools.RetryRule;
...
public abstract class CameraIntegrationTest<E extends CameraBaseEngine> extends BaseTest {
    ...
    public void testOpenClose() {
        Assert.assertEquals(CameraState.OFF, controller.getState());
        openSync(true);
    ...
    public void testSetWhiteBalance() {
        ...
        for (WhiteBalance value : values) {
            ...
            op.await(300);
            if (options.supports(value)) {
                Assert.assertEquals(camera.getWhiteBalance(), value);
                oldValue = value;
            } else {
                Assert.assertEquals(camera.getWhiteBalance(), oldValue);
            }
        }
    }
    ...
    public void testSetAudio() {
        ...
        for (Audio value : values) {
            ...
            Assert.assertEquals(camera.getAudio(), value);
        }
    }
